...
void LCD_Command(uint8_t cmd)
{
  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_6, GPIO_PIN_RESET); // RS = 0
...
// Send data (char) to LCD (RS=1)
void LCD_Data(uint8_t data)
{
  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_6, GPIO_PIN_SET);   // RS = 1
  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_0, GPIO_PIN_RESET); // RW = 0
...
void LCD_Send4Bits(uint8_t nibble)
{
	...
	HAL_GPIO_WritePin(GPIOD, GPIO_PIN_4, (nibble >> 3) & 0x01);
	    ...
	    // Pulse E pin PD1
	    HAL_GPIO_WritePin(GPIOD, GPIO_PIN_7, GPIO_PIN_SET); // Use PD7 as E
	    HAL_Delay(0.4);
